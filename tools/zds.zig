//! Zaxon Discussion (ZDS) management tool.
//!
...
const usage_text =
    ...
    \\                   entries.
    \\
    \\Slugs are lowercase words separated by hyphens, as in `wal-compaction`.
    \\
;
...
fn promote(
    ...
    const bundle_entry = try std.fmt.allocPrint(gpa,
        ...
        \\
    , .{
        &number, slug, &number, title, summary,
        &number, slug, &number, slug,  &number,
        slug,
    });
